use crate::endpoint::{
    Endpoint, EndpointAddress, EndpointDirection, EndpointSyncType, EndpointType,
};
...
impl<B: UsbBus> UsbBusAllocator<B> {
    ...
    pub fn alloc<'a, D: EndpointDirection>(
        ...
        //TODO: EP Sync type to alloc_ep may be needed.
        self.bus
            .borrow_mut()
    ...
    pub fn control<D: EndpointDirection>(&self, max_packet_size: u16) -> Endpoint<'_, B, D> {
        self.alloc(
            ...
            EndpointSyncType::NoSynchronization,
            max_packet_size,
            0,
        )
    ...
    /// # Arguments
    ///
    /// * `max_packet_size` - Maximum packet size in bytes. Must be one of 8, 16, 32, 64 or 128,
    ///
    /// # Panics
    ///
    /// Panics if endpoint allocation fails, because running out of endpoints or memory is not
